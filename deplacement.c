...
Liste_possibilitees *Possibilite_modifie(int Plateau[][8] ,int X ,int Y,Liste_possibilitees *liste){
    /*fonction qui remplit une liste chaînée de cases possibles pour une pièce entrée
            ...
            *p_e_p =e_p; /*e_p est modifié dans remplissage cases_pions si il y a une prise en passant
            ...
            while(transfert!=NULL){
                if(e_p==0){
                    ...
                    de la pièce entrée dans toutes ses possibilités et on regarde avec la fonction test echec si on
                    est en échec après le changement de position si oui, on supprime la possibilité car on ne
                    ...
                    Remplacer_abs_ord(Plateau,transfert->abscisse,transfert->ordonnee,X,Y,&val_remplacement);/*puis une deuxième pour revenir à l'état initial
