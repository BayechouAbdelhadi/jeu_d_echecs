    ...
    *sup='0';
    ...
    if(lettre_ou_chiffre(entree[0])==1 && lettre_ou_chiffre(entree[1])==-1){//case où le pion avance tout droit, promotion possible
        ...
        *pile_fin = empiler(*pile_fin,casep);
        ...
        if(lettre_ou_chiffre(entree[2])==-1 || lettre_ou_chiffre(entree[1])==0){//tour déplacement avec manger pas d'ambiguité
        ...
        else if(lettre_ou_chiffre(entree[2])==1){//déplaceùent ambigui sans manger
        ...
        else if(lettre_ou_chiffre(entree[2])==0){//manger vec ambiguité
            colonne_ou_ligne = adaptateur(entree[1]);
